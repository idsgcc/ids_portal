...
TENDER_URL    = "https://isupplier.nama.om:4443/OA_HTML/OA.jsp?OAFunc=XXTEN_TENDER_FLOAT_OETC_F"
...
def fetch_tenders():
    ...
    for row in soup.find_all("tr"):
        ...
        if len(cells) >= 6 and cells[0].startswith("OETC/"):
            try:
...
def fetch_known_numbers():
    ...
    while True:
        resp = requests.get(
            ...
            params={"select": "number", "source": "eq.oetc"},
            timeout=15,
        )
        resp.raise_for_status()
        batch = resp.json()
...
def save_tenders(tenders):
    ...
    records = []
    for t in tenders:
        rec = {"number": t["number"], "title": t["title"], "seen_date": today, "source": "oetc"}
...
def main():
    ...
    print("Fetching tenders from OETC portal...")
    try:
        ...
        send_error_email(
            "OETC Scraper Error",
            f"The OETC tender scraper failed at {run_time.strftime('%Y-%m-%d %H:%M UTC')}.\n\nError: {msg}\n\nPortal: https://ids-portal-gold.vercel.app/tenders",
        )
        sys.exit(0)
